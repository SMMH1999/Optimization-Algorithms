...
function [Best_Hike,Best_Position,Convergence_curve]=HOA_v2(hiker,MaxIter,LB,UB,dim,ObjFun)
%% Problem Parameters
...
Convergence_curve = zeros(1,MaxIt);      % vectorize variable store fitness value per iteration
...
Convergence_curve(1) = min(fit);           % stores initial fitness value at 0th iteration
...
%         Sweaper_Hiker(i) = isd;
    for j = 1:nPop
     ...
     Convergence_curve(i) = min(fit); % store best fitness per iteration
