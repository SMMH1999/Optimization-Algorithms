            ...
            if ~isempty(pop{m})
                for n = 1:length(pop)
                    if (m ~= n && ~isempty(pop{m}) && ~isempty(pop{n}))
                        ...
                        if pop{m}{r_m}.cost < pop{n}{q_n}.cost
                            %% S1

                            mutant = clusters{m}.cluster_leader.position + F * (pop{m}{r_m}.best_position - pop{n}{q_n}.best_position);
                            mutant = min(max(mutant, VarMin), VarMax);

                            % Crossover
                            trial = pop{n}{q_n}.position;
                            j_rand = randi(nVar);
                            for j = 1:nVar
                                if rand <= CR || j == j_rand
                                    trial(j) = mutant(j);
                            ...
                            % Selection
                            trial_fitness = CostCalculator(trial, Cost_Function, Function_Number, costFunctionDetails);

                            if trial_fitness < pop{n}{q_n}.cost
                                if trial_fitness < pop{n}{q_n}.best_cost
                                    pop{n}{q_n}.best_cost = trial_fitness;
                                    pop{n}{q_n}.best_position = trial;
                                end
                                pop{n}{q_n}.position = trial;
                                pop{n}{q_n}.cost = trial_fitness;
                            end
                            ...
                            %% S2
                            mutant = clusters{n}.cluster_leader.position + F * (pop{n}{q_n}.best_position - pop{m}{r_m}.best_position);
                            mutant = min(max(mutant, VarMin), VarMax);

                            % Crossover
                            trial = pop{m}{r_m}.position;
                            j_rand = randi(nVar);
                            for j = 1:nVar
                                if rand <= CR || j == j_rand
                                    trial(j) = mutant(j);
                            ...
                            if trial_fitness < pop{m}{r_m}.cost
                                if trial_fitness < pop{m}{r_m}.best_cost
                                    pop{m}{r_m}.best_cost = trial_fitness;
                                    pop{m}{r_m}.best_position = trial;
                                end
                                pop{m}{r_m}.position = trial;
                                pop{m}{r_m}.cost = trial_fitness;
