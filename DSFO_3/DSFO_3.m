function [Best_Elite_Fitness, Best_Elite_Position, Convergence_Curve] = DSFO_3(LB, UB, Dim, Predator_no, Max_iter, Cost_Function, Function_Number, costFunctionDetails)
    ...
    for Itr = 1:Max_iter
        ...
        for i = 1:Prey_no
            ...
            S1 = rand() * (Elite_Position - Sardine_Position(idxs(1), :) + Attack_Power);
            S2 = rand() * (Elite_Position - Sardine_Position(idxs(2), :) + Attack_Power);
            S3 = rand() * (Elite_Position - Sardine_Position(idxs(3), :) + Attack_Power);

            % Mutant_Sardine_Position = Sardine_Position(idxs(1), :) + F * (Sardine_Position(idxs(2), :) - Sardine_Position(idxs(3), :));
            Mutant_Sardine_Position = S1 + F * (S2 - S3);
        ...
        if Elite_Fitness > Injured_Sardine_Fitness
            ...
            SailFish_Fitness(Elite_Index(1, 1), :) = Injured_Sardine_Fitness;
